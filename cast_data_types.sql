...
-- orders_types data type conversion
...
-- dim_users data type conversion
...
-- dim_store_details - data type conversion
...
-- dim_store_details - Change Web Store row value from null to N/A
-- UPDATE dim_store_details SET latitude = 'N/A', longitude = 'N/A' WHERE store_type = 'Web Portal';


-- dim_products extra cleaning and add new column 'weight_class'
...
-- dim_products data type conversion and rename column 'removed' to 'still_available'
ALTER TABLE dim_products
...
-- dim_date_times data type conversion
...
-- dim_card_details data type conversion
...
-- Add primary keys to each of the tables prefixed with 'dim'
...
-- resolving issues with the foreign key (card_number)
...
-- Finalising the star-based schema & adding the foreign keys to the orders_table
ALTER TABLE orders_table
