...
class DataCleaning:
    ...
    def clean_card_data(df):
        ...
        df.loc[:, 'expiry_date'] = pd.to_datetime(
        ...
        df = df.loc[
            df['date_payment_confirmed'].notna() &
            (df['date_payment_confirmed'] < today_date)
        ]
    ...
    def clean_date_times_data(df):
        ...
        df['time_period'] = df['time_period'].where(
            df['time_period'].isin(valid_time_periods))
    ...
    def clean_products_data(df):
        ...
        df.loc[df['category'].str.contains(r'\d', na=False), 'category'] = None
        df.replace({'removed': {'Still_avaliable': 'Still_available'}},
                   inplace=True)
        df.loc[~df['removed'].isin(['Removed', 'Still_available']),
               'removed'] = None
    ...
    def clean_store_data(df):
        ...
        df['continent'] = (
            df['continent'].where(df['continent'].isin(['America', 'Europe']))
        )
        df['staff_numbers'] = (
            pd.to_numeric(df['staff_numbers'], errors='coerce')
            .astype(pd.Int64Dtype())
        )
        df = DataCleaning.clean_country_codes(df)
        df = DataCleaning.remove_cells_with_letters(
    ...
    def clean_user_data(df):
        df = (df.pipe(DataCleaning.clean_alphabetical_columns)
                ...
                .assign(date_of_birth=lambda df: df['date_of_birth']
                        .apply(DataCleaning.parse_date))
                .assign(join_date=lambda df: df['join_date']
                        .apply(DataCleaning.parse_date))
...
# ----- Other helper functions -----
    ...
    def clean_alphabetical_columns(df):
        ...
        df[['first_name', 'last_name']] = (
            df[['first_name', 'last_name']].applymap(clean_func)
        )
        return df
    ...
    def clean_phone_numbers(df):
        def clean_number(row):
            ...
            country_code = country_code_dict.get(row['country_code'], '')
            if raw_number.startswith(country_code):
                raw_number = raw_number[len(country_code):]

            raw_number = country_code + raw_number

            try:
                parsed_number = phonenumbers.parse(
                    raw_number, row['country_code'])
                formatted_number = phonenumbers.format_number(
                    parsed_number,
                    phonenumbers.PhoneNumberFormat.INTERNATIONAL)
                return (formatted_number if phonenumbers.is_valid_number(
                    parsed_number) else pd.NA)
    ...
    def convert_product_weights(weight):
        if isinstance(weight, str):
            if 'kg' in weight:
                ...
                unit_weight_cleaned = float(
                    re.sub('[^0-9.]', '', unit_weight))
                if 'g' in unit_weight:
                    return float(quantity) * (unit_weight_cleaned / 1000)
                else:
                    return float(quantity) * unit_weight_cleaned
        else:
            return weight
    ...
    def parse_date(date_str):
        try:
            return parse(
                date_str).strftime('%Y-%m-%d') if isinstance(
                    date_str, str) else np.nan
        except ParserError:
            return np.nan
    ...
    def clean_country_codes(df):
        ...
        df.loc[~df['country_code'].isin(
            ['US', 'GB', 'DE']), 'country_code'] = None
        return df

    @staticmethod
    def remove_cells_with_letters(df, columns):
        df[columns] = df[columns].applymap(
            lambda x: np.nan if isinstance(x, str) and re.search(
                r'[a-zA-Z]', x) else x)
        return df
